...
class ReducedEnv(Env):
    def __init__(self, plot_results = False):
        ...
        # observation space has the form: [pv, mv, error, error_integral]
        lower_limits = np.array([MIN_SETPOINT, MIN_CONTROL_ACTION, -MAX_ERROR, 0])
        upper_limits = np.array([MAX_SETPOINT, MAX_CONTROL_ACTION, MAX_ERROR, MAX_ERROR * SIMULATION_MAX_ITERATIONS*SIMULATION_STEP_PERIOD_SEC])
        self.observation_space = Box(np.array(lower_limits), np.array(upper_limits), shape=(4,), dtype=np.float16)
    ...
    def __serialize_state(self):
        ...
        output = np.array([pv, mv, error, error_integral]).astype(np.float16)
